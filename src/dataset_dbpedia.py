...
import pickle
#lanyun-tmp/mscrs/conv_data/redial/dbpedia_subkg.json
class DBpedia:
    def __init__(self, dataset, debug=False):
        self.debug = debug
        self.dataset_dir = os.path.join('/root/autodl-tmp/MSCRS/rec_data', dataset)
    ...
    def _process_entity_kg(self, SELF_LOOP_ID = 185):
        ...
        # The released MSCRS code uses two different KG contracts.  ReDial
        # filters very rare relations and remaps their ids, whereas INSPIRED
        # keeps the relation ids already stored in relation2id.json.  Loading
        # INSPIRED through the ReDial branch silently changes the graph and
        # makes its pretrained prompt checkpoint incompatible.
        ...
        # relation_idx intentionally starts at 1 in the original MSCRS data.
        # RGCNConv expects every edge type to be < num_relations, therefore the
        # correct vocabulary size is max_id + 1 (id 0 remains reserved).
...
class Co_occurrence:
    def __init__(self, dataset,split, entity_max_length, all_items,n_entity,debug=False):
        ...
        input_file = os.path.join(
            '/root/autodl-tmp/MSCRS/rec_data', dataset, 'edge_index_c.pt'
        )
